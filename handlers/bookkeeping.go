...
func (gh *GRCHandler) ProcessTransactionsHandler(w http.ResponseWriter, r *http.Request, db *badger.DB) {
	...
	result, err := vault.RunWithResult(vaultDir, ledgerDir)
	if err != nil {
	...
	webhookInfo := map[string]interface{}{
		...
		"succeeded": result.Dispatch.Succeeded,
		"failed":    result.Dispatch.Failed,
	}
	if len(result.Dispatch.Errors) > 0 {
		webhookInfo["errors"] = result.Dispatch.Errors
	}
